...
- generates personalized daily interpretations via Gemini API
...
- **ai**: Gemini API (google genai sdk) for interpretation, with template fallback
- **auth**: JWT + bcrypt
...
pip install fastapi uvicorn pyswisseph sqlalchemy google-genai pyjwt bcrypt python-dotenv pydantic httpx

cp .env.example .env
# add your GEMINI_API_KEY to .env (optional, falls back to templates)
...
| `GEMINI_API_KEY` | no | enables AI interpretations via Gemini (falls back to templates without it) |
...
6. **interpreter.py** feeds the overlay into Gemini for personalized plain-language interpretation
